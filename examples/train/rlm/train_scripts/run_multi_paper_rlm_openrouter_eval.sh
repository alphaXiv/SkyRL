...
# No GPUs required — all generation (parent + child agents) is routed through
# the OpenRouter API via the frozen_openrouter_model config.
#
# 1. Set OPENROUTER_API_KEY in your environment.
# 2. Create data (if needed): uv run --python 3.12 -- python examples/train/rlm/datasets/rlm_dataset_synthetic_multi.py --output_dir $DATA_DIR
# 3. Run: bash examples/train/rlm/train_scripts/run_multi_paper_rlm_openrouter_eval.sh
...
uv run --python 3.12 --extra skyrl-train -m examples.train.rlm.main_rlm_eval \
  ...
  trainer.run_name="rlm_multi_paper_openrouter_eval" \
