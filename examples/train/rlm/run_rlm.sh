...
uv run --extra fsdp -m skyrl.train.entrypoints.main_base \
  ...
  generator.child_inference_engine.num_engines=$CHILD_NUM_ENGINES \
  generator.child_inference_engine.tensor_parallel_size=$CHILD_TP_SIZE \
  generator.child_inference_engine.gpu_memory_utilization=0.6 \
  generator.child_inference_engine.async_engine=true \
  generator.child_inference_engine.enforce_eager=false \
  generator.child_inference_engine.backend=$INFERENCE_BACKEND \
  generator.child_inference_engine.engine_init_kwargs.language_model_only=true \
